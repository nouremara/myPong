...
CourtScene::CourtScene(Game& game) :
    ...
    mRightPaddle(*this, game.getResolution()[0] - EDGE_OFFSET - BOX_WIDTH / 2, (game.getResolution()[1] / 2) - (PADDLE_HEIGHT / 2), BOX_WIDTH / 2,
        PADDLE_HEIGHT),
    ...
    mLeftScoreIndicator(game, game.getHalfResolution()[0] - (70 + (game.getResolution()[0] / 10)), 10, game.getResolution()[0] / 15,
        game.getResolution()[1] / 20),
    mRightScoreIndicator(game, game.getHalfResolution()[0] + 70, 10, game.getResolution()[0] / 15, game.getResolution()[1] / 20),
    mRemainingPauseTicks(PAUSE_TICKS)
{
  mBackgroundImage = mGame.createImage("./assets/boardBackground.png");
}
...
void CourtScene::onDraw(SDL_Renderer& renderer)
{
  ...
  SDL_RenderCopy(&renderer, mGame.createText(mGame.getCurrentUser().userName.c_str()), nullptr, &rect);

  // draw the lives indicator
  rect.x = 700;  rect.y = 555;  rect.w = 100;  rect.h = 30;
  SDL_RenderCopy(&renderer, mGame.createImage("./assets/lifes.png"), nullptr, &rect);
...
void CourtScene::onKeyDown(SDL_KeyboardEvent& event)
{
  switch (event.keysym.sym) {
    case SDLK_w:    mLeftPaddle.setMovement(Paddle::Movement::UP);    break;
    case SDLK_s:    mLeftPaddle.setMovement(Paddle::Movement::DOWN);  break;
    case SDLK_UP:   mRightPaddle.setMovement(Paddle::Movement::UP);   break;
    case SDLK_DOWN: mRightPaddle.setMovement(Paddle::Movement::DOWN); break;
  }
}
...
void CourtScene::addPlayerScore(int playerIndex)
{
  ...
  mRemainingPauseTicks = PAUSE_TICKS;
  if (scores[playerIndex] > myPong::MAX_GOALS_TO_WIN) { mGame.setScene(std::make_shared<EndgameScene>(mGame)); }
}
